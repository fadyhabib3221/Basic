...
// Cell background colors (used for the highlighted totals row and the striped rows in
// exported ticket sheets) can't be written by the plain "xlsx" package — SheetJS's free
// Community Edition only supports writing colors in its paid Pro version. "xlsx-js-style"
// is a maintained, MIT-licensed fork with the exact same API that adds free style
// writing, so it's used here as a drop-in replacement. Run `npm install xlsx-js-style`
// (and `npm uninstall xlsx` if you like) for this to work.
...
export default function TicketsApp({ onChangeServer, currentServerUrl } = {}) {
  ...
  // Cell fill/font styles used by styleTicketSheet below.
  const XLSX_HEADER_STYLE = { fill: { patternType: "solid", fgColor: { rgb: "115E59" } }, font: { color: { rgb: "FFFFFF" }, bold: true } };
  const XLSX_TOTAL_STYLE = { fill: { patternType: "solid", fgColor: { rgb: "FFF59D" } }, font: { bold: true } };
  const XLSX_STRIPE_STYLE = { fill: { patternType: "solid", fgColor: { rgb: "F2F2F2" } } };

  // Colors an exported sheet so it's easier to scan: the header row gets a solid dark
  // fill with white bold text, every other data row gets a light gray stripe, and (when
  // the sheet ends in a totals row, i.e. dataRowCount includes it) the very last row is
  // highlighted in yellow. headerRow/dataRowCount are 0-based row indices into the sheet.
  const styleTicketSheet = (ws, { headerRow, dataRowCount, colCount, hasTotalRow = true }) => {
    const lastDataRow = headerRow + dataRowCount - 1;
    for (let r = headerRow; r <= lastDataRow; r++) {
      const isHeader = r === headerRow;
      const isTotal = hasTotalRow && r === lastDataRow;
      const isStripe = !isHeader && !isTotal && (r - headerRow) % 2 === 1;
      const style = isHeader ? XLSX_HEADER_STYLE : isTotal ? XLSX_TOTAL_STYLE : isStripe ? XLSX_STRIPE_STYLE : null;
      if (!style) continue;
  ...
  const sheetWithFilterBanner = (rows, filterLabel) => {
    ...
    styleTicketSheet(ws, { headerRow: 2, dataRowCount: rows.length, colCount });
    return ws;
  };
  ...
  const exportAllMonths = () => {
    const wb = XLSX.utils.book_new();
    const summaryRows = monthlyBreakdown.map((m) => ({
    ...
    const summarySheet = XLSX.utils.json_to_sheet(summaryRows);
    styleTicketSheet(summarySheet, { headerRow: 0, dataRowCount: summaryRows.length, colCount: 4, hasTotalRow: false });
